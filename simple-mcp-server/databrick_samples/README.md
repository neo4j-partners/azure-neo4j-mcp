...
1. **MCP Server Deployed**: The Neo4j MCP server must be running on Azure Container Apps (with a `MCP_ACCESS.<name>.json` file generated by `deploy.sh`)
2. **Databricks Runtime**: 15.4 LTS or later, or SQL warehouse 2023.40+
3. **Unity Catalog**: Must be enabled on your workspace
4. **Databricks CLI**: Installed and authenticated (`databricks auth login`)

### Connection Naming Convention

The connection name is derived from the `.env` file suffix:
...
From your local machine (where `MCP_ACCESS.<name>.json` exists):
...
### Option 2: Manual Setup via Catalog Explorer

Use the `MCP_ACCESS.<name>.json` file in the project root to manually configure the connection in Databricks:
