"""
Sync Azure deployment outputs and MCP configuration to .env file.
...
# Variables synced from MCP_ACCESS.json
MCP_MANAGED_VARS = {
    'MCP_ENDPOINT',
    'MCP_API_KEY',
}
...
def load_mcp_config() -> dict[str, str]:
    """
    Load MCP configuration from MCP_ACCESS.json in project root.

    Returns:
        Dictionary with MCP_ENDPOINT and MCP_API_KEY, or empty dict if not found.
    """
    ...
    try:
        ...
        api_key = mcp_access.get("api_key", "")
        ...
        print(f"Warning: Failed to parse MCP_ACCESS.json: {e}")
...
def main():
    ...
    try:
        ...
        # Get MCP configuration from MCP_ACCESS.json
        print("2. Loading MCP configuration from MCP_ACCESS.json...")
        mcp_config = load_mcp_config()
        if mcp_config:
            print(f"   Found: {MCP_ACCESS_FILE}")
        else:
            print(f"   Not found: {MCP_ACCESS_FILE}")
            print("   (Deploy MCP server from project root first)")

        # Combine all managed variables
        all_managed = {**azd_managed, **mcp_config}
        ...
        if ENV_FILE.exists():
            ...
            updated_vars = set()
            for var_name, value in all_managed.items():
            ...
            # Append new MCP variables that don't exist
            new_mcp_vars = set(mcp_config.keys()) - updated_vars
            if new_mcp_vars:
                if lines and lines[-1].strip():
                    lines.append('')
                lines.append('# MCP Server (from MCP_ACCESS.json)')
                for var_name in sorted(new_mcp_vars):
                    lines.append(f'{var_name}={mcp_config[var_name]}')
                    existing_vars[var_name] = mcp_config[var_name]
            ...
            if mcp_config:
                lines.append('')
                lines.append('# MCP Server (from MCP_ACCESS.json)')
                for key in sorted(mcp_config.keys()):
                    lines.append(f'{key}={mcp_config[key]}')

            # Add all placeholder sections
            existing_vars = dict(all_managed)
            lines = add_placeholder_sections(lines, existing_vars)
        ...
        for key in ['AZURE_AI_SERVICES_ENDPOINT', 'AZURE_AI_MODEL_NAME']:
        ...
        print("\nMCP configuration:")
        if mcp_config:
            endpoint = mcp_config.get('MCP_ENDPOINT', '')
            print(f"  MCP_ENDPOINT={endpoint[:50]}..." if len(endpoint) > 50 else f"  MCP_ENDPOINT={endpoint}")
            print("  MCP_API_KEY=<set>")
        else:
        ...
        if not mcp_config:
            print("\nNext steps:")
            print("  1. Deploy MCP server: ./scripts/deploy.sh (from project root)")
            print("  2. Run this script again to sync MCP configuration")
        elif not neo4j_configured:
            print("\nNext steps:")
            print("  1. Update Neo4j credentials in .env")
            print("  2. Run: cd langgraph-mcp-agent && uv run python simple-agent.py")
        else:
            print("\nReady to run samples!")
            print("  cd langgraph-mcp-agent && uv run python simple-agent.py")
